<script>
  ...
  import * as api from '$lib/api/recursos';
  ...
  let formData = $state({ nombre: '', descripcion: '' });
  ...
  function openCreate() {
    ...
    formData = { nombre: '', descripcion: '' }; formErrors = {}; showModal = true;
  }

  function openEdit(r) {
    editing = true; editId = r.id;
    formData = { nombre: r.nombre, descripcion: r.descripcion || '' }; formErrors = {}; showModal = true;
  }
  ...
  async function save() {
    ...
    saving = true;
    try {
      if (editing) { await api.actualizar(editId, formData); showNotification('Recurso actualizado'); }
      else { await api.crear(formData); showNotification('Recurso creado'); }
      showModal = false; await loadData();
    } catch (e) { showNotification(e.message, 'error'); }
    finally { saving = false; }
  }

  function confirmDelete(r) { deleteTarget = r; }

  async function handleDelete() {
    if (!deleteTarget) return;
    try {
      await api.eliminar(deleteTarget.id); showNotification('Recurso eliminado');
...
<div class="max-w-6xl mx-auto space-y-6">
  <header class="flex justify-between items-start">
    <div>
      <h2 class="h2 m-0">Recursos</h2>
      <p class="opacity-60 text-sm mt-1">Catálogo de recursos físicos disponibles</p>
    </div>
    {#if isAdmin}
  ...
  <div class="card p-5">
    <DataTable {columns} rows={recursos} {loading} emptyText="No hay recursos registrados">
  ...
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" onclick={() => showModal = false} role="dialog" aria-modal="true">
    <div class="card p-6 w-full max-w-md mx-4" onclick={(e) => e.stopPropagation()}>
      <div class="flex justify-between items-center mb-4">
        <h3 class="h3 m-0">{editing ? 'Editar Recurso' : 'Nuevo Recurso'}</h3>
        <button class="btn btn-ghost btn-sm" onclick={() => showModal = false}><X size={18} /></button>
      </div>
      <form onsubmit={save} class="space-y-4">
        <div>
          <label class="label">Nombre *</label>
          <input type="text" bind:value={formData.nombre} class="input w-full" placeholder="Nombre del recurso" />
...
{#if deleteTarget}
  <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" onclick={() => deleteTarget = null} role="dialog" aria-modal="true">
    <div class="card p-6 w-full max-w-sm mx-4" onclick={(e) => e.stopPropagation()}>
      <h3 class="h3 m-0 mb-2">Eliminar Recurso</h3>
